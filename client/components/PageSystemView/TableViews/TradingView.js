import React from 'react'

const TradingView = () => {
	return (
		<div className="box box-transparent">
			<div className="box-body">
				<br/>
				<div className="box box-default table-box table-responsive mdl-shadow--2dp">
					<table className="mdl-data-table">
						<thead className="tbl-header">
							<tr>
								<th className="mdl-data-table__cell--non-numeric">Date / Time</th>
								<th className="mdl-data-table__cell--non-numeric">Exchange / Source</th>
								<th className="mdl-data-table__cell--non-numeric">Action</th>
								<th className="mdl-data-table__cell--non-numeric">Coin</th>
								<th>Volume</th>
								<th>Price</th>
								<th>Fees</th>
								<th>Cost / Proceeds</th>
							</tr>
						</thead>
						<tbody className="tbl-body">
							<tr>
								<td className="mdl-data-table__cell--non-numeric">23/06/2016 09:56:45</td>
								<td className="mdl-data-table__cell--non-numeric">Coinbase</td>
								<td className="mdl-data-table__cell--non-numeric">BUY</td>
								<td className="mdl-data-table__cell--non-numeric">BTC</td>
								<td>1.20582968</td>
								<td>$656.87550</td>
								<td>$7.92000</td>
								<td>$-800.0</td>
							</tr>
							
							<tr>
								<td className="mdl-data-table__cell--non-numeric">23/06/2016 09:56:45</td>
								<td className="mdl-data-table__cell--non-numeric">Coinbase</td>
								<td className="mdl-data-table__cell--non-numeric">BUY</td>
								<td className="mdl-data-table__cell--non-numeric">BTC</td>
								<td>1.20582968</td>
								<td>$656.87550</td>
								<td>$7.92000</td>
								<td>$-800.0</td>
							</tr>
							
							<tr>
								<td className="mdl-data-table__cell--non-numeric">23/06/2016 09:56:45</td>
								<td className="mdl-data-table__cell--non-numeric">Coinbase</td>
								<td className="mdl-data-table__cell--non-numeric">BUY</td>
								<td className="mdl-data-table__cell--non-numeric">BTC</td>
								<td>1.20582968</td>
								<td>$656.87550</td>
								<td>$7.92000</td>
								<td>$-800.0</td>
							</tr>
							
							<tr>
								<td className="mdl-data-table__cell--non-numeric">23/06/2016 09:56:45</td>
								<td className="mdl-data-table__cell--non-numeric">Coinbase</td>
								<td className="mdl-data-table__cell--non-numeric">BUY</td>
								<td className="mdl-data-table__cell--non-numeric">BTC</td>
								<td>1.20582968</td>
								<td>$656.87550</td>
								<td>$7.92000</td>
								<td>$-800.0</td>
							</tr>
							
							<tr>
								<td className="mdl-data-table__cell--non-numeric">23/06/2016 09:56:45</td>
								<td className="mdl-data-table__cell--non-numeric">Coinbase</td>
								<td className="mdl-data-table__cell--non-numeric">BUY</td>
								<td className="mdl-data-table__cell--non-numeric">BTC</td>
								<td>1.20582968</td>
								<td>$656.87550</td>
								<td>$7.92000</td>
								<td>$-800.0</td>
							</tr>
						</tbody>
					</table>
				</div>
			</div>
		</div>
	)
}

export default TradingView